...
CREATE TABLE edx.dataframe (
    ...
    forum_f_reading_ease NUMERIC,
    forum_fk_readling_level NUMERIC,
    forum_net_votes INT
);
...
COMMENT ON COLUMN edx.dataframe.quiz_peer_views IS 'Number  of  pageviews  of  human-graded  quiz  pages,  as  measured  byclickstream features';
COMMENT ON COLUMN edx.dataframe.quiz_lead_time IS 'Time between a quiz submission and deadline for all submissions; discretizedbuckets for t≥7 days (2), 3≤t<7 (1), 1≤t<3, 0≤t<1 (0), and late (-1)';
...
COMMENT ON COLUMN edx.dataframe.quiz_percent_allowed_submissions IS 'Total count of quiz submissions as a percent of the maximum allowedsubmissions';
